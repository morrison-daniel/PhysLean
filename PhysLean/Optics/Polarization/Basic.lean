...
noncomputable def harmonicElectromagneticPlaneWave (k : WaveVector) (E₀x E₀y ω δx δy : ℝ)
    ...
    ElectricField :=
    fun t r => Ex k E₀x ω δx t r • EuclideanSpace.single 0 1 +
    Ey k E₀y ω δy t r • EuclideanSpace.single 1 1
...
lemma harmonicElectromagneticPlaneWave_eq_electricplaneWave {c : ℝ} {k : WaveVector}
  ...
  unfold harmonicElectromagneticPlaneWave Ex Ey electricPlaneWave
...
/-- Alternative representation for `Ex` to simplify calculations. -/
noncomputable def Ex (E₀x τ δx: ℝ) := E₀x * cos (τ + δx)

/-- Alternative representation for `Ey` to simplify calculations. -/
noncomputable def Ey (E₀y τ δy: ℝ) := E₀y * cos (τ + δy)

/-- Representation for `Ex` is equivalent with `τ = ω * t - inner ℝ k r`. -/
lemma eq_Ex (k : WaveVector) (E₀x ω δx : ℝ) (t : Time) (r : Space) :
    Ex E₀x (ω * t - inner ℝ k r) δx = Optics.Ex k E₀x ω δx t r := by
  rw [Ex, Optics.Ex, harmonicWave, sub_add]

/-- Representation for `Ey` is equivalent with `τ = ω * t - inner ℝ k r`. -/
lemma eq_Ey (k : WaveVector) (E₀y ω δy : ℝ) (t : Time) (r : Space) :
    Ey E₀y (ω * t - inner ℝ k r) δy = Optics.Ey k E₀y ω δy t r := by
  rw [Ey, Optics.Ey, harmonicWave, sub_add]
...
theorem polarizationEllipse {E₀x E₀y τ δx δy : ℝ} (hx : E₀x ≠ 0) (hy : E₀y ≠ 0) :
    ((Ex E₀x τ δx)/E₀x)^2 + ((Ey E₀y τ δy)/E₀y)^2 -
    2 * (Ex E₀x τ δx) * (Ey E₀y τ δy) * cos (δy - δx) / (E₀x * E₀y) =
    sin (δy - δx) ^ 2 := by
  have h1 : (Ex E₀x τ δx)/E₀x * sin δy - (Ey E₀y τ δy)/E₀y * sin δx =
      cos τ * sin (δy - δx) := by
    field_simp
    rw [Ex, Ey, cos_add, cos_add, sin_sub]
    ring
  have h2 : (Ex E₀x τ δx)/E₀x * cos δy - (Ey E₀y τ δy)/E₀y * cos δx =
      sin τ * sin (δy - δx) := by
    field_simp
    rw [Ex, Ey, cos_add, cos_add, sin_sub]
    ring
  trans (Ex E₀x τ δx / E₀x) ^ 2 * (sin δy ^ 2 + cos δy ^ 2) +
      (Ey E₀y τ δy / E₀y) ^ 2 * (sin δx ^ 2 + cos δx ^ 2) -
      2 * Ex E₀x τ δx * Ey E₀y τ δy * cos (δy - δx) / (E₀x * E₀y)
  · simp
