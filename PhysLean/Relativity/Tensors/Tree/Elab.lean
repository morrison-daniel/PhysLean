...
- `{T | μ ν}ᵀᵀ` is `tensorNode T`.
- We can also write e.g. `{T | μ ν}ᵀᵀ.tensor` to get the tensor itself.
- `{- T | μ ν}ᵀᵀ` is `neg (tensorNode T)`.
- `{T | 0 ν}ᵀᵀ` is `eval 0 0 (tensorNode T)`.
- `{T | μ ν + T' | μ ν}ᵀᵀ` is `addNode (tensorNode T) (perm _ (tensorNode T'))`, where
  here `_` will be the identity permutation so does nothing.
- `{T | μ ν = T' | μ ν}ᵀᵀ` is `(tensorNode T).tensor = (perm _ (tensorNode T')).tensor`.
- If `a ∈ k` then `{a •ₜ T | μ ν}ᵀᵀ` is `smulNode a (tensorNode T)`.
- If `g ∈ G` then `{g •ₐ T | μ ν}ᵀᵀ` is `actionNode g (tensorNode T)`.
- Suppose `T2` is a tensor `S.F (OverColor.mk ![c3])`.
  Then `{T | μ ν ⊗ T2 | σ}ᵀᵀ` is `prodNode (tensorNode T1) (tensorNode T2)`.
- If `T3` is a tensor `S.F (OverColor.mk ![S.τ c1, S.τ c2])`, then
  `{T | μ ν ⊗ T3 | μ σ}ᵀᵀ` is `contr 0 1 _ (prodNode (tensorNode T1) (tensorNode T3))`.
  `{T | μ ν ⊗ T3 | μ ν }ᵀᵀ` is
  `contr 0 0 _ (contr 0 1 _ (prodNode (tensorNode T1) (tensorNode T3)))`.
- If `T4` is a tensor `S.F (OverColor.mk ![c2, c1])` then
  `{T | μ ν + T4 | ν μ }ᵀᵀ`is `addNode (tensorNode T) (perm _ (tensorNode T4))` where `_`
  is the permutation of the two indices of `T4`.
  `{T | μ ν = T4 | ν μ }ᵀᵀ` is `(tensorNode T).tensor = (perm _ (tensorNode T4)).tensor` is the
...
- In all of theses expressions `μ`, `ν` etc are free. It does not matter what they are called,
  Lean will elaborate them in the same way. In other words, `{T | μ ν ⊗ T3 | μ ν }ᵀᵀ` is exactly
  the same to Lean as `{T | α β ⊗ T3 | α β }ᵀᵀ`.
...
def indexToIdent (stx : Syntax) : TermElabM Ident :=
  match stx with
  | `(indexExprTT|$a:ident) => return a
  | `(indexExprTT| τ($a:ident)) => return a
  | _ =>
    throwError "Unsupported expression syntax in indexToIdent: {stx}"

/-- Takes a pair ``a b : ℕ × TSyntax `indexExprTT``. If `a.1 < b.1` and `a.2 = b.2` then
  outputs `some (a.1, b.1)`, otherwise `none`. -/
def indexPosEq (a b : TSyntax `indexExprTT × ℕ) : TermElabM (Option (ℕ × ℕ)) := do
...
def indexToDual (stx : Syntax) : Bool :=
  match stx with
  | `(indexExprTT| τ($_)) => true
  | _ => false

/-!

## Manipulation of lists of indexExprTT
...
/-- For list of `indexExprTT` e.g. `[α, 3, β, 2, γ]`, `getEvalPos`
...
def getEvalPos (ind : List (TSyntax `indexExprTT)) : TermElabM (List (ℕ × ℕ)) := do
  let indEnum := ind.zipIdx
  let evals := indEnum.filter (fun x => indexExprTTIsNum x.1)
  let evals2 ← (evals.mapM (fun x => indexToNum x.1))
  let pos := evalAdjustPos (evals.map (fun x => x.2))
  return List.zip pos evals2

/-- For list of `indexExprTT` e.g. `[α, 3, β, α, 2, γ]`, `getContrPos`
...
def getContrPos (ind : List (TSyntax `indexExprTT)) : TermElabM (List (ℕ × ℕ)) := do
  let indFilt : List (TSyntax `indexExprTT) := ind.filter (fun x => ¬ indexExprTTIsNum x)
...
def withoutContrEval (ind : List (TSyntax `indexExprTT)) :
    TermElabM (List (TSyntax `indexExprTT)) := do
  let indFilt : List (TSyntax `indexExprTT) := ind.filter (fun x => ¬ indexExprTTIsNum x)
...
def getPermutation (l1 l2 : List (TSyntax `indexExprTT)) : TermElabM (List (ℕ)) := do
...
def getPermutationTerm (l1 l2 : List (TSyntax `indexExprTT)) : TermElabM Term := do
...
declare_syntax_cat tensorExprTT

/-- The syntax for a tensor node. -/
syntax term "|" (ppSpace indexExprTT)* : tensorExprTT

/-- Equality. -/
syntax:40 tensorExprTT "=" tensorExprTT:41 : tensorExprTT

/-- The syntax for tensor prod two tensor nodes. -/
syntax:70 tensorExprTT "⊗" tensorExprTT:71 : tensorExprTT

/-- The syntax for tensor addition. -/
syntax tensorExprTT "+" tensorExprTT : tensorExprTT

/-- Allowing brackets to be used in a tensor expression. -/
syntax "(" tensorExprTT ")" : tensorExprTT

/-- Scalar multiplication for tensors. -/
syntax term "•ₜ" tensorExprTT : tensorExprTT

/-- group action for tensors. -/
syntax term "•ₐ" tensorExprTT : tensorExprTT
...
/-- For syntax of the form `T | α β 2 β`, `getAllIndices` returns a list `[α, β, 2, β]`
  of all `indexExprTT`. -/
def getAllIndices (stx : Syntax) : TermElabM (List (TSyntax `indexExprTT)) := do
  match stx with
  | `(tensorExprTT| $_:term | $[$args]*) => do
      let indices ← args.toList.mapM fun arg => do
        match arg with
        | `(indexExprTT|$t:indexExprTT) => pure t
      return indices
  | _ =>
...
-/
partial def getProdIndices (stx : Syntax) : TermElabM (List (TSyntax `indexExprTT)) := do
  match stx with
  | `(tensorExprTT| $_:term | $[$args]*) => do
      return (← withoutContrEval (← getAllIndices stx))
  | `(tensorExprTT| $a:tensorExprTT ⊗ $b:tensorExprTT) => do
      let indicesA ← withoutContrEval (← getProdIndices a)
      let indicesB ← withoutContrEval (← getProdIndices b)
      return indicesA ++ indicesB
  | `(tensorExprTT| ($a:tensorExprTT)) => do
...
-/
partial def getIndicesFull (stx : Syntax) : TermElabM (List (TSyntax `indexExprTT)) := do
  match stx with
  | `(tensorExprTT| $_:term | $[$args]*) => do
      return (← withoutContrEval (← getAllIndices stx))
  | `(tensorExprTT| $_:tensorExprTT ⊗ $_:tensorExprTT) => do
      return (← withoutContrEval (← getProdIndices stx))
  | `(tensorExprTT| ($a:tensorExprTT)) => do
      return (← getIndicesFull a)
  | `(tensorExprTT| -$a:tensorExprTT) => do
      return (← getIndicesFull a)
  | `(tensorExprTT| $_:term •ₜ $a) => do
      return (← getIndicesFull a)
  | `(tensorExprTT| $a:tensorExprTT + $_:tensorExprTT) => do
      return (← getIndicesFull a)
  | _ =>
...
def getIndicesLeft (stx : Syntax) : TermElabM (List (TSyntax `indexExprTT)) := do
  match stx with
  | `(tensorExprTT| $a:tensorExprTT + $_:tensorExprTT) => do
      return (← getIndicesFull a)
  | _ =>
...
def getIndicesRight (stx : Syntax) : TermElabM (List (TSyntax `indexExprTT)) := do
  match stx with
  | `(tensorExprTT| $_:tensorExprTT + $a:tensorExprTT) => do
      return (← getIndicesFull a)
  | _ =>
...
def getIndicesLeftEq (stx : Syntax) : TermElabM (List (TSyntax `indexExprTT)) := do
  match stx with
  | `(tensorExprTT| $a:tensorExprTT = $_:tensorExprTT) => do
      return (← getIndicesFull a)
  | _ =>
...
def getIndicesRightEq (stx : Syntax) : TermElabM (List (TSyntax `indexExprTT)) := do
  match stx with
  | `(tensorExprTT| $_:tensorExprTT = $a:tensorExprTT) => do
      return (← getIndicesFull a)
  | _ =>
...
partial def syntaxFull (stx : Syntax) : TermElabM Term := do
  ...
  | `(tensorExprTT| $c:term •ₜ $a:tensorExprTT) => do
      return smulTermMap c (← syntaxFull a)
  | `(tensorExprTT| $c:term •ₐ $a:tensorExprTT) => do
...
def elaborateTensorNode (stx : Syntax) : TermElabM Expr := do
  let tensorExprTT ← elabTerm (← syntaxFull stx) none
  return tensorExprTT

/-- The tensor tree corresponding to a tensor expression. -/
syntax (name := tensorExprTTSyntax) "{" tensorExprTT "}ᵀᵀ" : term

elab_rules (kind:=tensorExprTTSyntax) : term
  | `(term| {$e:tensorExprTT}ᵀᵀ) => do
