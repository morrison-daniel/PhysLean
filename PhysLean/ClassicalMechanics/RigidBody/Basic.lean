...
/-- We can describe the motion of rigid body with a fixed (inertial) coordinate system (X,Y,Z) and 
    a moving system (x₁,x₂,x₃) rigidly attached to the body --/
informal_definition coordinate_system :
  tag := "LL31.3"
  deps := [``RigidBody]
...
informal_lemma velocity_decomposition :
  tag := "LL31.L3"
  deps := [``RigidBody]

/-- The angular velocity of rotation of a rigid body from a system of coordinates fixed in the body 
    is independent of the system chosen. -/
...
/-- The motion of a rigid body can be decomposed into a translation of some reference point plus a 
    rotation about that point. There exists a time-dependent vector V(t) and angular velocity ω(t) such
    that v(r) = V + ω × r, where r is measured from the reference point. -/
...
/-- The total angular momentum about a point O is L = ∫ r × v dm. With v = V + ω × r about the centre of mass,
    L = R × (M V) + I_CM ω, where R is the centre of mass position. -/
...
/-- In the inertial frame, the translational equation of motion of a rigid body is given by  
    dP/dt = F, where `P` is the total linear momentum and `F` is the total external force acting on the body. -/
informal_lemma translational_equation_inertial where
  tag := "LL32-TR"
  deps := [``RigidBody

/-- In the inertial frame, the rotational equation of motion of a rigid body about the center of mass is given by  
    dM/dt = K,  
    where `M` is the total angular momentum and `K` is the total external torque. -/
...
/-- Because the inertia tensor is real symmetric, there exists an orthonormal basis of principal axes 
in which it is diagonal. The corresponding directions are the principal axes of inertia. -/
...
informal_lemma principal_axes_of_inertia_bound
  tag := "LL32-PAEB"
  deps := [``RigidBody]

/-- An asymmetrical top is when none of the principal moments of inertia are equal -/ 
informal_definition asymmetrical_top
  tag := "LL32-AST"
  deps := [``RigidBody]

/-- An symmetrical top is when only two of the principal moments of inertia are equal -/ 
informal_definition asymmetrical_top
  tag := "LL32-ST"
  deps := [``RigidBody]

/-- An spherical top is when all three of the principal moments of inertia are equal -/ 
informal_definition asymmetrical_top
  tag := "LL32-SPT"
  deps := [``RigidBody]
...
/-- For any vector field A(t),
    its inertial-frame time derivative equals the rotating-frame derivative
    plus the contribution from the frame rotation:
...
/-- For linear momentum, the relation between inertial and rotating derivatives is
      (dP/dt)_inertial = d'P/dt + Ω × P. So, 
      d'P/dt + Ω × P = F which is
    the linear-momentum equation in the rotating frame. -/
informal_lemma transport_law_for_momentum where
  tag := "LL32.L.transportP"
  deps := [``RigidBody]

/-- For angular momentum, the relation between inertial and rotating derivatives
    is
...
/-- When motion is described in body-fixed principal axes (I₁, I₂, I₃ diagonal), the equations of 
    rotational motion (Euler’s equations) are: I₁ dω₁/dt + (I₃ − I₂) ω₂ ω₃ = M₁, with cyclic permutations.
    M is the external torque about the centre of mass. -/
informal_lemma euler_equations where
  tag := "LL32-EQ"
  deps := [``RigidBody]

/-- A rigid body can perform steady (uniform) rotation about any principal axis if the torque about that axis vanishes. 
    Stability depends on the ordering of principal moments. -/
...
/-- The power delivered to a rigid body by forces is P = ∑ Fᵢ ⋅ vᵢ = F_tot ⋅ V + M ⋅ ω, where F_tot is total force, 
    V the reference point velocity, and M the torque. Translational and rotational contributions separate. -/
informal_lemma rigid_body_work_and_power where
tag := "LL32-WP"
deps := [``RigidBody]

/-- Small oscillations about a stable equilibrium orientation are governed by linearised equations obtained by 
    expanding energy to second order in angular displacements. Normal modes and frequencies depend on inertia and restoring torques. -/
