...
@[simp] lemma ofNNReal_val (t : ℝ≥0) : (ofNNReal t).val = t := rfl
@[simp] lemma coe_ofNNReal_coe (t : ℝ≥0) : ((ofNNReal t : Temperature) : ℝ≥0) = t := rfl
@[simp] lemma coe_ofNNReal_real (t : ℝ≥0) : ((ofNNReal t : Temperature) : ℝ) = t := rfl

/-- Convenience: build a temperature from a real together with a proof of nonnegativity. -/
@[simp] noncomputable def ofRealNonneg (t : ℝ) (ht : 0 ≤ t) : Temperature :=
  ofNNReal ⟨t, ht⟩

@[simp] lemma ofRealNonneg_val {t : ℝ} (ht : 0 ≤ t) :
...
lemma Beta_fun_T_formula (t : ℝ) (ht : 0 < t) :
  ...
  have : ((Temperature.ofNNReal (Real.toNNReal t)).β : ℝ) = 1 / (kB * t) := by
    simp [Temperature.β, Temperature.ofNNReal, Temperature.toReal,
      Real.toNNReal_of_nonneg ht0, one_div, mul_comm, mul_left_comm, mul_assoc]
  simpa [Beta_fun_T] using this

/-- On `Ioi 0`, `Beta_fun_T t` equals `1 / (kB * t)`. -/
lemma Beta_fun_T_eq_on_Ioi :
    EqOn Beta_fun_T (fun t : ℝ => 1 / (kB * t)) (Set.Ioi 0) := by
  intro t ht
  exact Beta_fun_T_formula t ht

lemma deriv_beta_wrt_T (T : Temperature) (hT_pos : 0 < T.val) :
    HasDerivWithinAt Beta_fun_T (-1 / (kB * (T.val : ℝ)^2)) (Set.Ioi 0) (T.val : ℝ) := by
  let f : ℝ → ℝ := fun t => 1 / (kB * t)
  have h_eq : EqOn Beta_fun_T f (Set.Ioi 0) := Beta_fun_T_eq_on_Ioi
...
lemma chain_rule_T_beta {F : ℝ → ℝ} {F' : ℝ}
    ...
    HasDerivWithinAt (fun t : ℝ => F (Beta_fun_T t))
      (F' * (-1 / (kB * (T.val : ℝ)^2))) (Set.Ioi 0) (T.val : ℝ) := by
  have hβ_deriv := deriv_beta_wrt_T (T:=T) hT_pos
  have h_map : Set.MapsTo Beta_fun_T (Set.Ioi 0) (Set.Ioi 0) := by
    intro t ht
    have ht_pos : 0 < t := ht
    have : 0 < 1 / (kB * t) := by
      have : 0 < kB * t := mul_pos kB_pos ht_pos
      exact one_div_pos.mpr this
    have h_eqt : Beta_fun_T t = 1 / (kB * t) := (Beta_fun_T_eq_on_Ioi) ht
    simpa [h_eqt] using this
  have h_beta_at_T : Beta_fun_T (T.val : ℝ) = (T.β : ℝ) := by
    have hTposR : 0 < (T.val : ℝ) := hT_pos
    have h_eqt := (Beta_fun_T_eq_on_Ioi) hTposR
    simpa [Temperature.β, Temperature.toReal] using h_eqt
  have hF_deriv' : HasDerivWithinAt F F' (Set.Ioi 0) (Beta_fun_T (T.val : ℝ)) := by
